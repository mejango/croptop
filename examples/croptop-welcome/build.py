...
ids.update({'writing': '9CB0B19C-061C-4378-9D27-9E434C796B76', 'audio': 'C7215628-0976-4CC6-AC20-7950C93A83F3', 'video': '77E6E2AE-B847-4BF2-B904-93E59021712E'})
...
wave_svg='<svg viewBox="0 0 640 140" aria-hidden="true">'+''.join(f'<line x1="{5+i*10}" x2="{5+i*10}" y1="{70-max(2,v*55)}" y2="{70+max(2,v*55)}" stroke="currentColor" stroke-width="2"/>' for i,v in enumerate(waveform))+'</svg>'
...
posts[1:1]=[
 ...
 {'key':'audio','title':'Audio','label':'Audio','tag':'posts','media':audio_uri,'waveform':waveform,'body':f'<h2>Small hours</h2><div class="ct-audio-wave">{wave_svg}</div><audio class="ct-media" controls preload="metadata" aria-label="Small hours, a twelve-second audio loop" src="{audio_uri}"></audio>'+source()},
 {'key':'video','title':'Video','label':'Video','tag':'posts','media':video_uri,'poster':poster_uri,'body':f'<video class="ct-media" controls playsinline preload="metadata" poster="{poster_uri}" aria-label="Passing light, an eight-second silent video" src="{video_uri}"></video>'+source()}
]

posts.append({'key':'bots','title':'Bots','label':'Bots','tag':'start here','body':(root/'content/bots.html').read_text()+source()})
order=['start','essays','audio','documents','video','writing','studio','art','bots','network','faq']
...
for n,p in enumerate(posts):
 ...
 preview_code=preview.replace('CONFIG',json.dumps({k:p[k] for k in ['key','label','order','media','waveform','poster'] if k in p}))
...
<script>{(root/'welcome.js').read_text()}</script></body></html>'''
 ...
 p.pop('body')
 for field in ['media','waveform','poster']:p.pop(field,None)
